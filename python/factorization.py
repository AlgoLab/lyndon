...
def d_duval(data, **kwargs):
    if type(data) == str:
        result = d_duval_(data, duval, **kwargs)
    else:
        result = []
        for word in data:
            result.append(d_duval_(word, duval, **kwargs))
      
    return result

def d_cfl(data, **kwargs):
    return d_duval(data, **kwargs)

def d_icfl(data, **kwargs):
    if type(data) == str:
        result = d_duval_(data, icfl, **kwargs)
    else:
        result = []
        for word in data:
            result.append(d_duval_(word, icfl, **kwargs))
    return result

def d_cfl_icfl(data, **kwargs):
    if type(data) == str:
        result = d_duval_(data, cfl_icfl, **kwargs)
    else:
        result = []
        for word in data:
            result.append(d_duval_(word, cfl_icfl, **kwargs))
    return result
